import numpy as np
import pytest
from banking_risk.frtb.csr.delta import SA_CSR_Delta_Calculator, CSR_Delta_Result
from banking_risk.frtb.constants import CSR_NONSEC_RISK_WEIGHTS

_CALC = SA_CSR_Delta_Calculator()
...
def test_K_scales_with_risk_weight():
    s1 = _CALC.compute(_sens(1, 1.0))   # RW = 0.5%
    s6 = _CALC.compute(_sens(6, 1.0))   # RW = 1.0%
    assert s6.K[6] > s1.K[1]
...
def test_single_bucket_capital_equals_K():
    result = _CALC.compute(_sens(8, 50.0))
    assert result.capital == pytest.approx(result.K[8], rel=1e-9)
