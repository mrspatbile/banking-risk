"""
FRTB SA CSR non-securitisation curvature — CRR3 Art. 325e/325ef.

Same Taylor-approximation formula as GIRR curvature but applied to
credit spread risk: CVR^± ≈ −0.5 × Σ_k γ_k × ΔRW_k²

where ΔRW_k is the CSR delta risk weight for the relevant bucket.
Aggregation uses the same cross-bucket γ = 0.05 as CSR delta.
...
_RW = np.array(CSR_NONSEC_RISK_WEIGHTS)
...
def curvature_pnl_from_greeks(
    ...
    """Approximate CSR curvature P&L for a single bucket.
    ...
    bucket : int
        CSR bucket number (1–18) — determines the risk weight.
...
class SA_CSR_Curvature_Calculator:
    """CRR3 Art. 325e/325ef CSR curvature capital."""

    def compute(
        self,
        cvr_up  : dict[int, float],
        cvr_down: dict[int, float],
    ) -> CSR_Curvature_Result:
