"""
FRTB SA CSR securitisation vega — CRR3 Art. 325bd.

Vega sensitivities on a 5×5 (option expiry × credit tenor) grid per bucket,
adapted for securitisation buckets (1–41).
...
_RHO_E    = _build_rho(CSR_SEC_VEGA_ALPHA, _E)
_RHO_T    = _build_rho(CSR_SEC_VEGA_ALPHA, _T)
_RHO_VEGA = np.kron(_RHO_E, _RHO_T)


@dataclass
class CSR_Sec_Vega_Result:
    ...
    def to_table(self) -> pd.DataFrame:
        ...
        for b in self.buckets:
            rows.append({"bucket": b, "desc": CSR_SEC_BUCKET_DESCS[b-1],
                         "K": self.K[b], "S": self.S[b]})
        return pd.DataFrame(rows).set_index("bucket")


class CSR_Sec_Vega_Calculator(ABC):
    @abstractmethod
    def compute(self, sensitivities: dict[int, np.ndarray]) -> CSR_Sec_Vega_Result: ...


class SA_CSR_Sec_Vega_Calculator(CSR_Sec_Vega_Calculator):
    """CRR3 Art. 325bd CSR sec vega calculator — 5×5 expiry×tenor grid per bucket."""
    ...
    def compute(self, sensitivities: dict[int, np.ndarray]) -> CSR_Sec_Vega_Result:
        ...
        for b in buckets:
            ...
            ws_flat = s.ravel() * CSR_SEC_VEGA_RISK_WEIGHT
            ws_dict[b] = pd.Series(
                ws_flat,
